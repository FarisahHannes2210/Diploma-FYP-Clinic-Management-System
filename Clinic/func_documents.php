...
if (isset($_POST['add_doct_btn'])) {
  ...
  $sql = "INSERT INTO document (DOCT_NAME, DOCT_FILE_PATH, DOCT_TYPE,DOCT_DESC) VALUES ('$DOCT_NAME','$file','$DOCT_TYPE','$DOCT_DESC')";
	...
	if(!mysqli_query($conn,$sql)){
	   ...
	   echo "<script>alert('File Upload Successfully!'); window.location.href='documents.php';</script>";
  }
// } else {
...
if(isset($_GET['DOCT_ID'])){
  ...
  // echo $a->output();
  //
  // $stat = $conn-> prepare("SELECT * FROM document WHERE DOCT_ID = $DOCT_ID");
  // $stat ->bindParam(1,$DOCT_ID);
  // $stat ->execute();
  // header("Content-Type: ".$row['project_name']);
  // echo $row['DOCT_DETAIL'];
